...
// Announcements collection helper
export async function getAnnouncementsCollection() {
  const db = await getDatabase();
  const collection = db.collection('announcements');
  await collection.createIndex({ classroomId: 1, createdAt: -1 });
  await collection.createIndex({ 'poll.options.voters': 1 });
  return collection;
}

// Announcement Activity collection helper
export async function getAnnouncementActivityCollection() {
  const db = await getDatabase();
  return db.collection('announcement_activity');
}
